...
function positionCells() {
    const numCells = boardCells.length;
    const radius = 180; // 圆形半径
    const centerX = 250; // 圆心X坐标
    const centerY = 250; // 圆心Y坐标

    boardCells.forEach((cell, index) => {
        const angle = (index * 360) / numCells; // 每个格子之间的角度
        const x = centerX + radius * Math.cos((angle * Math.PI) / 180);
        const y = centerY + radius * Math.sin((angle * Math.PI) / 180);

        cell.style.transform = `translate(-50%, -50%) translate(${x - centerX}px, ${y - centerY}px)`;
    });
}
...
function movePlane(plane, currentPosition, diceRoll) {
    const targetCell = boardCells[(currentPosition + diceRoll) % boardCells.length];
